###################################################################################################
## NCHDA specific procedures (SP) algorithm is written by John Stickley
## SP algorithm is developed and used by National Institute for Cardiovascular Outcomes Research (NICOR) (version 8.05)
...
sp_algorithm <- function(sp) {
  ...
  num_diag <- 20
  diagnosiscodes_fields <- sprintf("diagcode%d", seq(1:num_diag))
  
  code_list <- as.list(sp[c(procedurecodes_fields)])
  
  ## currently used for valve repairs to exclude AVSD
  diagnosis_list <- as.list(sp[c(diagnosiscodes_fields)])
  ...
  if ((sp["type_procedure"] %in% type_procedure == TRUE) & (any(code_1 %in% code_list)) &
      (!any(exclude %in% code_list))) {
    sp_algorithm_group <- "23:ross_konno_a"
    return(sp_algorithm_group)
  }
